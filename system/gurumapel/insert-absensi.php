						...
						<div class="table-responsive">
							<table id="table-sm" class="table table-bordered table-hover table-sm">
								...
								<tbody>
									...
									$no=0;
									$i=1;
									$query=mysql_query("SELECT s.nama, s.nis, s.nm_kelas, a.ket FROM siswa s LEFT JOIN absensi a ON s.nis = a.nis WHERE s.nm_kelas='$nm_kelas' AND (a.tanggal='$tanggal' OR a.tanggal IS NULL) ORDER BY s.nis ASC");
									...
									<?php
									echo " <input type='radio' name='absen-$data[nis]' value='hadir' id='$no'"; if($data['ket'] == "H") echo 'checked'; echo "><label for='$no'>Hadir  </label>";
									$no++;
									echo " <input type='radio' name='absen-$data[nis]' value='sakit' id='$no'"; if($data['ket'] == "S") echo 'checked'; echo "><label for='$no'>Sakit  </label>";
									$no++;
									echo " <input type='radio' name='absen-$data[nis]' value='ijin' id='$no'"; if($data['ket'] == "I") echo 'checked'; echo "><label for='$no'>Ijin  </label>";
									$no++;
									echo " <input type='radio' name='absen-$data[nis]' value='alfa' id='$no'"; if($data['ket'] == "A") echo 'checked'; echo "><label for='$no'>Alfa  </label>";
									$no++;
									?>
									</td>
